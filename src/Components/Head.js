...
export const Head = () => {
  ...
  const getSearchSuggestions = async () => {
    try {
      ...
      const response = await fetch(
        `https://cors-anywhere.herokuapp.com/https://suggestqueries.google.com/complete/search?client=firefox&ds=yt&q=${searchQuery}`
      );
  ...
  const handleSearchClick = (selectedQuery) => {
    setSearchQuery(selectedQuery);
    dispatch(addSearchQuery(selectedQuery));
  ...
  <svg id="yt-logo-updated_yt1" viewBox="0 0 90 20" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
    ...
    <g>
      ...
      <div className='col-span-10 px-10 xl:mx-36 relative'>
        <form 
          onSubmit={(e) => {
            e.preventDefault(); 
        ...
        >
          <input
            className='w-1/2 p-2 px-5 border border-black rounded-l-full'
            value={searchQuery}
          ...
          />
          <button 
            className='px-5 py-2 border border-black rounded-r-full' 
            type='submit'
          >
            🔍
          </button>
        ...
        {showSuggestions && suggestions.length > 0 && (
          <div className='absolute bg-white py-2 px-5 w-1/2 shadow-lg rounded-lg border-gray-100'>
            <ul>
